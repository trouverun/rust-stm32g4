...
fn main() {
    ...
    let multisine_path = std::path::Path::new(&out_dir).join("multisine_table.rs");
    std::fs::write(&multisine_path, emit_multisine_table()).expect("write multisine_table.rs");

    println!("cargo:rerun-if-changed={}", dbc_path);
}

/// Unit-amplitude multisine, one fundamental period, Schroeder phases.
fn emit_multisine_table() -> String {
    ...
    let mut s = String::from("// AUTOGENERATED\n\n");
    writeln!(s, "pub const MULTISINE_HARMONICS: [u32; {}] = {:?};", HARMONICS.len(), HARMONICS).unwrap();
    writeln!(s, "pub const MULTISINE_TABLE: [f32; {TICKS}] = [").unwrap();
    for v in &samples {
        writeln!(s, "    {:?},", (v / peak) as f32).unwrap();
    }
    writeln!(s, "];").unwrap();
    s
}
